...
export default function DashBoard({ clientHistory, serverHistory, toolHistory }: DashboardProps) {
  const renderHistory = (title: string, history: string[]) => (
    <div className="flex-1 bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 flex flex-col gap-4 shadow-2xl h-full overflow-hidden">
      <h2 className="text-xl font-bold bg-gradient-to-r from-[#00f2fe] to-[#4facfe] bg-clip-text text-transparent">{title}</h2>
  ...
  return (
    <div className="text-white p-8 h-full flex flex-col max-w-[90rem] mx-auto">
      <h1 className="text-4xl font-bold mb-8 bg-gradient-to-r from-[#A855F7] to-[#6366F1] bg-clip-text text-transparent">Talos Dashboard</h1>
